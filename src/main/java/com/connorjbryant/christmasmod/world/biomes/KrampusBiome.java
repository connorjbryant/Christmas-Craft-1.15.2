...
public class KrampusBiome extends Biome {
	
	public KrampusBiome(Builder biomeBuilder) {
		super(biomeBuilder);
		this.addSpawn(EntityClassification.MONSTER, new Biome.SpawnListEntry(EntityType.SKELETON, 10, 2, 5));
		...
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
		 ...
		 this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
		 Feature.NORMAL_TREE.withConfiguration(DimensionTree.DIMENSION_TREE_CONFIG).withPlacement(
		 Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(3,
		 0.1f, 1))));
