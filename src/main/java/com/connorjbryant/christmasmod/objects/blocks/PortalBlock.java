...
public class PortalBlock extends Block {
	...
	public void onEntityCollision(BlockState state, World worldIn, BlockPos pos, Entity entityIn) {
		if (!entityIn.isPassenger() && !entityIn.isBeingRidden() && entityIn.isNonBoss()) {
			if (entityIn.dimension != DimensionType.byName(ChristmasMod.CHRISTMAS_DIM_TYPE)) {
				teleportToDimension(entityIn, DimensionType.byName(ChristmasMod.CHRISTMAS_DIM_TYPE), pos);
			} else {
	...
	public static class Size {
		...
		protected boolean func_196900_a(BlockState pos) {
			...
			return pos.isAir() || block == Blocks.FIRE || block == BlockInit.PORTAL.get();
		}
			...
			for (int i = 0; i < this.width; ++i) {
				...
				for (int j = 0; j < this.height; ++j) {
					this.world.setBlockState(blockpos.up(j),
							BlockInit.PORTAL.get().getDefaultState().with(PortalBlock.AXIS, this.axis), 18);
				}
			}
